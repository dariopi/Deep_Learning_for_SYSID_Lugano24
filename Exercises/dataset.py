...
class F16DS(Dataset):
  ...
  def __init__(self, pd_file, na, nb, ind_out):
    ...
    for i in range(n, len(pd_file) - n):
      ...
      self.regressor[i - n, nb + 1:nb + nb +
                     2] = pd_file.loc[i:i - nb:-1, "Voltage"]
      self.regressor[i - n, nb + nb + 2:nb + nb + 2 +
                     na] = pd_file.loc[i - 1:i - na:-1, output_names[ind_out]]
  ...
  def __init__(self, pd_file, seq_len, ind_out):
    ...
    for ind in range(n):
      if ind < n - 1:
        ...
        u[ind, :, 1] = pd_file.iloc[ind *
                                    seq_len:(ind + 1) * seq_len]["Voltage"]
  ...
  def __init__(self, pd_file, seq_len, ind_out=None):
    ...
    for ind in range(n):
      if ind < n - 1:
        ...
        u[ind, :, 0] = pd_file.loc[ind *
                                   seq_len:(ind + 1) * seq_len - 1]["uBenchMark"]

        y[ind, :, 0] = pd_file.loc[ind *
                                   seq_len:(ind + 1) * seq_len - 1]["yBenchMark"]
...
if __name__ == "__main__":
  ...
  F16DS_train = F16DS_seq(
      pd_file=dict_ds["train"][55000:70000], seq_len=23, ind_out=[0, 1])
